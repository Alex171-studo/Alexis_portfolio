...
def brute_force_scanner():
    ...
    parser.add_argument("--seuil",type=int,default=5,help="Seuil de détection brute force (défaut : 5)")
    ...
    with open (report_file,'w') as report:
        ...
        if suspect_ips:
            ...
            for ip in sorted(suspect_ips.keys()):
                report.write(f" {ip} : {suspect_ips.get(ip)} tentatives -{status(suspect_ips.get(ip))}\n")
        if normal_ips:
